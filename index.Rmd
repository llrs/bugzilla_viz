...
So the question is who is contributing so much. Who are the most contributing users and how are they contributing? I'll focus on the last 3 years (before the database dump)
...
begin <- max(opener$time, na.rm = TRUE) - lubridate::years(3)
...
active_users_merged |> 
    mutate(real_name = if_else(is.na(real_name), "", real_name)) |> 
    knitr::kable(
        col.names = c("ID", "Name", "All comments", "All attachments",
                      "Comments", "Attachments", "Submission", "Actions"))
```
...
Sebastian Meyer who has recently become a R core member is on the top of the  list by number of actions and attachments provided to bugs not open by him. 
...
Most bug reports where users are not engaged (do not reply to comments) is due to it being handled (closed or assigned) on that comment.
...
ue |> 
    ...
    labs(x = "Handled", y = "Engagement", 
         title = "Engagement of users on their bugs", size = "Users", 
         col = "Bugs")
```
