...
Omitting the warning, which are some bugs without date of creation.
...
The other two bugs are probably incorrectly moved bugs. 
But let's explore where and how where the bugs moved:
...
Not clear what happens on [1261](https://bugs.r-project.org/show_bug.cgi?id=1261) or [1605](https://bugs.r-project.org/show_bug.cgi?id=1605). But it seems that there were some [troubles migrating](https://stat.ethz.ch/pipermail/r-devel/2010-March/056954.html) the bugs and it is not completely clear when the switch happened. 
...
We can check out the missing issues and spam to see what happened:
...
But I think that the last number of spam the day a new SPAM policy was enacted shows that the numbers show mostly spam.
...
The measure to ask permission for an account (the vertical line) has worked very well as there seem to be less missing/spam bugs lately. 
Given all that we will omit the spam bugs from now on. 
They are not really bug reports nor report or have something of quality.
...
Perhaps some attachments weren't moved from Jitterbug, but it seems that the large difference might be because of this.
...
Also Major bugs have more changes of having a patch. 
Probably this is the reporter of the bug or people proposing solutions. 
...
attachment_time %>% 
  ...
  knitr::kable(col.names = c("Attachment on  opening", "Bugs", "%"))
```
...
db_activity2 %>% 
  ...
  labs(x = "Active", y = "Bugs", title = "Activity on bugs")
```
...
Most comments on bugs are from 2 different people. Presumably one is the author and another user.
